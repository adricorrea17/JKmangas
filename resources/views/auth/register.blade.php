...
<section class="margen col-12 rounded bg-dark p-5 text-light w-50 mx-auto">
    ...
    <form action="{{ route('auth.register.accion') }}" method="post" class="d-flex flex-column justify-content-center">
        ...
        <div class="mb-3">
            ...
            <input type="text" placeholder="Ingresa un nombre de usuario" name="nombre_usuario" id="nombre_usuario" class="form-control @error('password') border border-danger @enderror" value="{{ old('nombre_usuario', '') }}" @error('nombre_usuario') aria-describedby="nombre_usuario-error" @enderror>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="password" placeholder="Ingresa una contraseña" name="password" id="password" class="form-control @error('password') border border-danger @enderror">
        </div>
        @error('password')
        <div class="mb-3 text-danger font"><span class="visually-hidden">Error:</span> {{ $message }}</div>
        @enderror
